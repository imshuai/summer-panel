...
async function buildXrayBestPingConfig(proxySettings, totalAddresses, chainProxy, outbounds, isFragment) {
    const remark = isFragment ? '💦 Palmate F - Best Ping 💥' : '💦 Palmate - Best Ping 💥';
...
async function buildXrayBestFragmentConfig(proxySettings, hostName, chainProxy, outbounds) {
    ...
    const config = buildXrayConfig(proxySettings, '💦 Palmate F - Best Fragment 😎', true, true, chainProxy, false, false);
...
async function buildXrayWorkerLessConfig(proxySettings) {
    const config = buildXrayConfig(proxySettings, '💦 Palmate F - WorkerLess ⭐', true, false, false, false, false);
...
export async function getXrayCustomConfigs(request, env, isFragment) {
    ...
    if (outProxy) {
        ...
        try {
            ...
            await env.palmate.put("proxySettings", JSON.stringify({
                ...proxySettings, 
                outProxy: '',
                outProxyParams: {}
            }));
