...
export async function getDataset(request, env) {
    ...
    try {
        proxySettings = await env.palmate.get("proxySettings", {type: 'json'});
        warpConfigs = await env.palmate.get('warpConfigs', {type: 'json'});
    } catch (error) {
        console.log(error);
...
export async function updateDataset (request, env) {
    ...
    if (!isReset) {
        try {
            currentSettings = await env.palmate.get("proxySettings", {type: 'json'});
        } catch (error) {
            console.log(error);
            throw new Error(`An error occurred while getting current KV settings - ${error}`);
        }
    } else {
        await env.palmate.delete('warpConfigs');
        newSettings = null;
    }
    ...
    try {    
        await env.palmate.put("proxySettings", JSON.stringify(proxySettings));          
    } catch (error) {
        console.log(error);
